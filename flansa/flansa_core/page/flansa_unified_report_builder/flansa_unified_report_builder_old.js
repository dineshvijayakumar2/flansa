...
class UnifiedReportBuilder {
    ...
    setup_inline_html() {
        ...
        this.$container.html(`
            ...
            <div class="unified-builder-content single-step">
                <!-- Single step layout with all sections visible -->
                <div class="builder-sections">
                    <!-- Table Selection Section -->
                    <div class="section-card">
                        <div class="section-header">
                            <h5><i class="fa fa-table text-primary"></i> 1. Table Selection</h5>
                        </div>
                        <div class="section-content">
                            <div class="form-group">
                                <select class="form-control" id="table-select">
                                    <option value="">Choose a table for your report...</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-card">
                        <div class="section-header">
                            <h5><i class="fa fa-columns text-success"></i> 2. Field Selection</h5>
                        </div>
                        <div class="section-content">
                            <div class="field-selector">
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="field-categories compact">
                                            <div class="available-fields-list" id="current-fields">
                                                <p class="text-muted">Select a table first</p>
                                            </div>
                                            <div class="available-fields-list" id="system-fields" style="margin-top: 10px;">
                                            </div>
                                            <div id="related-fields-groups" style="margin-top: 10px;">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-8">
                                        <h6>Selected Fields <span class="badge badge-primary" id="selected-field-count">0</span></h6>
            ...
            <style>
                ...
                .field-item.selected-compact:hover {
                    background: #d4edff;
    ...
    select_field(e) {
        ...
        if (field && !this.selected_fields.find(f => f.fieldname === fieldname)) {
            ...
            const fieldCopy = { 
                ...field, 
                // Don't set custom_label initially - let it be undefined so we can distinguish between original and custom
        ...
        if (selectedContainer) {
            if (this.selected_fields.length === 0) {
                ...
                this.selected_fields.forEach((field, index) => {
                    const fieldItem = document.createElement('div');
                    fieldItem.className = 'field-item selected-compact';
                    fieldItem.dataset.fieldname = field.fieldname;
                    fieldItem.dataset.fieldIndex = index;
                    
                    const displayLabel = field.custom_label || field.field_label || field.label || field.fieldname;
                    const isCustomLabel = field.custom_label && field.custom_label !== (field.field_label || field.label || field.fieldname);
                    
                    fieldItem.innerHTML = `
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="flex: 1;">
                                <div class="field-label-display" style="display: flex; align-items: center; gap: 8px;">
                                    <strong class="field-display-name">${displayLabel}</strong>
                                    ${isCustomLabel ? '<span class="custom-label-indicator" title="Custom label">📝</span>' : ''}
                                    <i class="fa fa-edit edit-label-btn" style="color: #007bff; cursor: pointer; font-size: 0.9em;" 
                                       title="Edit label for this report" data-field-index="${index}"></i>
                                </div>
                                <div style="font-size: 0.8em; color: #666;">
                                    ${field.fieldtype}
                            ...
                            <i class="fa fa-times remove-field-btn" style="color: #dc3545; cursor: pointer;" 
                               title="Remove field" data-field-index="${index}"></i>
                        </div>
                        
                        <!-- Hidden edit form -->
                        <div class="label-edit-form" style="display: none; margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 4px;">
                            <div class="form-group" style="margin-bottom: 10px;">
                                <label style="font-size: 0.85em; color: #666; margin-bottom: 5px;">Custom Label for Report:</label>
                                <input type="text" class="form-control form-control-sm label-edit-input" 
                                       value="${displayLabel}" 
                                       placeholder="Enter custom label">
                            </div>
                            <div style="display: flex; gap: 8px;">
                                <button class="btn btn-sm btn-primary save-label-btn" data-field-index="${index}">
                                    <i class="fa fa-check"></i> Save
                                </button>
                                <button class="btn btn-sm btn-secondary cancel-label-btn" data-field-index="${index}">
                                    <i class="fa fa-times"></i> Cancel
                                </button>
                                ${isCustomLabel ? `<button class="btn btn-sm btn-outline-warning reset-label-btn" data-field-index="${index}">
                ...
                // Bind events for the new compact interface
                this.bind_selected_field_events();
            }
        }
    ...
    bind_selected_field_events() {
        ...
        container.querySelectorAll('.edit-label-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.show_label_edit_form(fieldIndex);
            });
        });
        ...
        container.querySelectorAll('.remove-field-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const fieldIndex = parseInt(btn.dataset.fieldIndex);
                this.remove_selected_field(fieldIndex);
            });
        });
        
        // Save label button
        container.querySelectorAll('.save-label-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const fieldIndex = parseInt(btn.dataset.fieldIndex);
                this.save_label_edit(fieldIndex);
            });
        });
        
        // Cancel label button
        container.querySelectorAll('.cancel-label-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const fieldIndex = parseInt(btn.dataset.fieldIndex);
                this.cancel_label_edit(fieldIndex);
            });
        });
        
        // Reset label button
        container.querySelectorAll('.reset-label-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const fieldIndex = parseInt(btn.dataset.fieldIndex);
                this.reset_label_edit(fieldIndex);
            });
        });
        
        // Enter key to save
        container.querySelectorAll('.label-edit-input').forEach(input => {
            input.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    const fieldIndex = parseInt(e.target.closest('.field-item').dataset.fieldIndex);
                    this.save_label_edit(fieldIndex);
                }
            });
        });
    ...
    show_label_edit_form(fieldIndex) {
        const fieldItem = document.querySelector(`[data-field-index="${fieldIndex}"]`);
        if (!fieldItem) return;
        
        const editForm = fieldItem.querySelector('.label-edit-form');
        const labelDisplay = fieldItem.querySelector('.field-label-display');
        
        if (editForm && labelDisplay) {
            editForm.style.display = 'block';
            labelDisplay.style.opacity = '0.5';
            
            // Focus the input
            const input = editForm.querySelector('.label-edit-input');
            if (input) {
                input.focus();
                input.select();
            }
    ...
    save_label_edit(fieldIndex) {
        const fieldItem = document.querySelector(`[data-field-index="${fieldIndex}"]`);
        if (!fieldItem || !this.selected_fields[fieldIndex]) return;
        
        const input = fieldItem.querySelector('.label-edit-input');
        if (!input) return;
        
        const newLabel = input.value.trim();
        if (newLabel) {
            this.selected_fields[fieldIndex].custom_label = newLabel;
            this.update_field_displays(); // Refresh the display
        }
    }
    
    cancel_label_edit(fieldIndex) {
        const fieldItem = document.querySelector(`[data-field-index="${fieldIndex}"]`);
        if (!fieldItem) return;
        ...
        if (editForm && labelDisplay) {
            ...
            labelDisplay.style.opacity = '1';
        }
    }
    
    reset_label_edit(fieldIndex) {
        if (!this.selected_fields[fieldIndex]) return;
        
        // Remove custom label to use original
        delete this.selected_fields[fieldIndex].custom_label;
        this.update_field_displays(); // Refresh the display
    }
    
    remove_selected_field(fieldIndex) {
        if (fieldIndex >= 0 && fieldIndex < this.selected_fields.length) {
            this.selected_fields.splice(fieldIndex, 1);
            this.update_field_displays();
        }
    }
    ...
    get_current_config() {
        ...
        return {
            selected_fields: this.selected_fields.map(f => ({
                fieldname: f.fieldname,
                category: f.category,
                fieldtype: f.fieldtype,
                field_label: f.field_label || f.label,
                custom_label: f.custom_label, // Save the custom label
                table: f.table,
                link_field: f.link_field,
        ...
        try {
            ...
            if (response.message && response.message.success) {
                ...
                setTimeout(() => {
                    ...
                    } else if (this.edit_report_id) {
                        // Editing existing report - go back to saved reports
                        window.location.href = `/app/flansa-saved-reports?table=${encodeURIComponent(this.selected_table)}`;
                    } else {
                        // New report - go to the report viewer to show the new report
                        const reportId = response.message.report_id;
                        window.location.href = `/app/flansa-report-viewer?report=${encodeURIComponent(reportId)}`;
                    }
                }, 1500);
            } else {
    ...
    populate_from_saved_report(report) {
        ...
        setTimeout(() => {
            if (report.config && report.config.selected_fields && this.available_fields) {
                ...
                const savedFields = report.config.selected_fields;
                const isOldFormat = savedFields.length > 0 && typeof savedFields[0] === 'string';
                
                this.selected_fields = [];
                
                if (isOldFormat) {
                    // Old format: array of fieldnames
                    this.selected_fields = allFields.filter(f => 
                        savedFields.includes(f.fieldname)
                    ).map(field => ({
                        ...field,
                        original_label: field.field_label || field.label || field.fieldname
                    }));
                } else {
                    // New format: array of field objects with custom labels
                    savedFields.forEach(savedField => {
                        const matchingField = allFields.find(f => f.fieldname === savedField.fieldname);
                        if (matchingField) {
                            this.selected_fields.push({
                                ...matchingField,
                                custom_label: savedField.custom_label, // Preserve the custom label
                                original_label: matchingField.field_label || matchingField.label || matchingField.fieldname
                            });
                        }
                    });
                }
