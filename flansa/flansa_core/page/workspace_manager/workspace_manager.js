...
class WorkspaceManager {
    ...
    async load_data() {
        try {
            ...
            const currentTenant = await this.call_api('get_current_workspace_info');
            this.render_current_tenant(currentTenant);
            
            // Load available workspaces
            const workspaces = await this.call_api('get_available_workspaces');
            this.render_tenant_list(workspaces, currentTenant.workspace_id);
    ...
    render_current_tenant(tenant) {
        ...
        $info.html(`
            <div class="row">
                <div class="col-md-6">
                    <h5>${tenant.workspace_name} <span class="current-tenant-badge">CURRENT</span></h5>
                    <p><strong>Workspace ID:</strong> <code>${tenant.workspace_id}</code></p>
    ...
    render_tenant_list(tenants, currentTenantId) {
        ...
        tenants.forEach(tenant => {
            const isCurrent = tenant.workspace_id === currentTenantId;
            ...
            html += `
                <div class="${cardClass}">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1" onclick="window.workspaceManager.switchWorkspace('${tenant.workspace_id}')" style="cursor: pointer;">
                            <h5>${tenant.workspace_name}${isCurrent ? '<span class="current-tenant-badge">CURRENT</span>' : ''}</h5>
                            <div class="tenant-id">ID: ${tenant.workspace_id}</div>
                            <div>Domain: ${tenant.primary_domain || 'N/A'}</div>
                            <div>Status: <span class="badge badge-${statusColor}">${tenant.status}</span></div>
                        </div>
                        <div class="tenant-actions">
                            <div class="btn-group-vertical btn-group-sm">
                                <button class="btn btn-outline-info" onclick="event.stopPropagation(); window.workspaceManager.viewWorkspaceStats('${tenant.workspace_id}')" title="View Statistics">
                                    <i class="fa fa-chart-bar"></i>
                                </button>
                                <button class="btn btn-outline-primary" onclick="event.stopPropagation(); window.workspaceManager.showEditWorkspaceForm('${tenant.workspace_id}')" title="Edit Workspace">
                                    <i class="fa fa-edit"></i>
                                </button>
                                ${tenant.status === 'Active' ? 
                                    `<button class="btn btn-outline-warning" onclick="event.stopPropagation(); window.workspaceManager.deactivateWorkspace('${tenant.workspace_id}')" title="Deactivate">
                                        <i class="fa fa-pause"></i>
                                    </button>` : 
                                    `<button class="btn btn-outline-success" onclick="event.stopPropagation(); window.workspaceManager.activateWorkspace('${tenant.workspace_id}')" title="Activate">
                                        <i class="fa fa-play"></i>
                                    </button>`
                                }
                                <button class="btn btn-outline-danger" onclick="event.stopPropagation(); window.workspaceManager.deleteWorkspace('${tenant.workspace_id}')" title="Delete Workspace">
                                    <i class="fa fa-trash"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                    ${!isCurrent && tenant.status === 'Active' ? '<button class="btn btn-sm btn-primary switch-btn mt-2" onclick="event.stopPropagation(); window.workspaceManager.switchWorkspace(\'' + tenant.workspace_id + '\')"><i class="fa fa-exchange"></i> Switch to this workspace</button>' : ''}
    ...
    async showEditWorkspaceForm(workspaceId) {
        try {
            ...
            this.showTenantForm(true, workspaceData);
        } catch (error) {
            frappe.msgprint({
                title: 'Error',
    ...
    showTenantForm(isEdit, tenantData) {
        ...
        const dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                },
                {
                    ...
                    label: 'Workspace ID',
                    default: tenantData?.workspace_id || '',
                ...
                },
                {
                    ...
                    reqd: 1,
                    default: tenantData?.workspace_name || ''
                },
                {
                    fieldtype: 'Select',
                    fieldname: 'status',
            ...
            primary_action: (values) => {
                ...
                if (isEdit) {
                    this.updateTenant(tenantData.workspace_id, values, dialog);
                } else {
        ...
        // Auto-generate workspace ID for new tenants
        if (!isEdit) {
            dialog.fields_dict.workspace_name.$input.on('input', () => {
                ...
                if (name) {
                    ...
                    dialog.set_value('workspace_id', tenantId);
                }
            });
        }
